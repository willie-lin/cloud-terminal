...
type RoleQuery struct {
	...
	withPermissionsBoundary *AccessPolicyQuery
	withFKs                 bool
...
func (_q *RoleQuery) QueryAccessPolicies() *AccessPolicyQuery {
...
func (_q *RoleQuery) QueryParentRole() *RoleQuery {
...
func (_q *RoleQuery) QueryChildRoles() *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryPermissionsBoundary chains the current query on the "permissions_boundary" edge.
func (_q *RoleQuery) QueryPermissionsBoundary() *AccessPolicyQuery {
	query := (&AccessPolicyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withPermissionsBoundary: _q.withPermissionsBoundary.Clone(),
...
func (_q *RoleQuery) WithAccessPolicies(opts ...func(*AccessPolicyQuery)) *RoleQuery {
	query := (&AccessPolicyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithParentRole tells the query-builder to eager-load the nodes that are connected to
// the "parent_role" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithParentRole(opts ...func(*RoleQuery)) *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withParentRole = query
	return _q
}

// WithChildRoles tells the query-builder to eager-load the nodes that are connected to
// the "child_roles" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithChildRoles(opts ...func(*RoleQuery)) *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withChildRoles = query
	return _q
}

// WithPermissionsBoundary tells the query-builder to eager-load the nodes that are connected to
// the "permissions_boundary" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithPermissionsBoundary(opts ...func(*AccessPolicyQuery)) *RoleQuery {
	query := (&AccessPolicyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPermissionsBoundary = query
	return _q
}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			_q.withParentRole != nil,
			_q.withChildRoles != nil,
			_q.withPermissionsBoundary != nil,
		}
	)
	if _q.withPermissionsBoundary != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withParentRole; query != nil {
		if err := _q.loadParentRole(ctx, query, nodes,
			func(n *Role) { n.Edges.ParentRole = []*Role{} },
			func(n *Role, e *Role) { n.Edges.ParentRole = append(n.Edges.ParentRole, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withChildRoles; query != nil {
		if err := _q.loadChildRoles(ctx, query, nodes,
			func(n *Role) { n.Edges.ChildRoles = []*Role{} },
			func(n *Role, e *Role) { n.Edges.ChildRoles = append(n.Edges.ChildRoles, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPermissionsBoundary; query != nil {
		if err := _q.loadPermissionsBoundary(ctx, query, nodes, nil,
			func(n *Role, e *AccessPolicy) { n.Edges.PermissionsBoundary = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *RoleQuery) loadAccessPolicies(ctx context.Context, query *AccessPolicyQuery, nodes []*Role, init func(*Role), assign func(*Role, *AccessPolicy)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.AccessPoliciesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *RoleQuery) loadParentRole(ctx context.Context, query *RoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.ParentRolePrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "parent_role" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *RoleQuery) loadChildRoles(ctx context.Context, query *RoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.ChildRolesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "child_roles" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *RoleQuery) loadPermissionsBoundary(ctx context.Context, query *AccessPolicyQuery, nodes []*Role, init func(*Role), assign func(*Role, *AccessPolicy)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Role)
	for i := range nodes {
		if nodes[i].role_permissions_boundary == nil {
			continue
		}
		fk := *nodes[i].role_permissions_boundary
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "role_permissions_boundary" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
