...
func (_q *AccountQuery) QueryUsers() *UserQuery {
...
func (_q *AccountQuery) QueryAccessPolicies() *AccessPolicyQuery {
...
func (_q *AccountQuery) QueryResource() *ResourceQuery {
...
func (_q *AccountQuery) WithUsers(opts ...func(*UserQuery)) *AccountQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *AccountQuery) WithAccessPolicies(opts ...func(*AccessPolicyQuery)) *AccountQuery {
	query := (&AccessPolicyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) WithResource(opts ...func(*ResourceQuery)) *AccountQuery {
	query := (&ResourceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Account, init func(*Account), assign func(*Account, *User)) error {
...
func (_q *AccountQuery) loadAccessPolicies(ctx context.Context, query *AccessPolicyQuery, nodes []*Account, init func(*Account), assign func(*Account, *AccessPolicy)) error {
...
func (_q *AccountQuery) loadResource(ctx context.Context, query *ResourceQuery, nodes []*Account, init func(*Account), assign func(*Account, *Resource)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Resource(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ResourceColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_resource
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_resource" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_resource" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
