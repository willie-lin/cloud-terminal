...
func (_q *GroupQuery) QueryAccessPolicies() *AccessPolicyQuery {
...
func (_q *GroupQuery) WithAccessPolicies(opts ...func(*AccessPolicyQuery)) *GroupQuery {
	query := (&AccessPolicyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAccessPolicies; query != nil {
		if err := _q.loadAccessPolicies(ctx, query, nodes,
			func(n *Group) { n.Edges.AccessPolicies = []*AccessPolicy{} },
			func(n *Group, e *AccessPolicy) { n.Edges.AccessPolicies = append(n.Edges.AccessPolicies, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedAccessPolicies {
		if err := _q.loadAccessPolicies(ctx, query, nodes,
			func(n *Group) { n.appendNamedAccessPolicies(name) },
			func(n *Group, e *AccessPolicy) { n.appendNamedAccessPolicies(name, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadAccessPolicies(ctx context.Context, query *AccessPolicyQuery, nodes []*Group, init func(*Group), assign func(*Group, *AccessPolicy)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.AccessPoliciesTable)
		joinT.Schema(_q.schemaConfig.GroupAccessPolicies)
		s.Join(joinT).On(s.C(accesspolicy.FieldID), joinT.C(group.AccessPoliciesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.AccessPoliciesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.AccessPoliciesPrimaryKey[0]))
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithNamedAccessPolicies(name string, opts ...func(*AccessPolicyQuery)) *GroupQuery {
	query := (&AccessPolicyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
