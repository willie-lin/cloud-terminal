...
type Asset struct {
	...
	// AssetDetails holds the value of the "asset_details" field.
...
func (*Asset) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case asset.FieldAssetName, asset.FieldAssetType, asset.FieldAssetDetails:
...
func (a *Asset) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case asset.FieldAssetDetails:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field asset_details", values[i])
			} else if value.Valid {
				a.AssetDetails = value.String
			}
		case asset.FieldGroupID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field group_id", values[i])
			} else if value.Valid {
				a.GroupID = int(value.Int64)
			}
		default:
...
func (a *Asset) String() string {
	...
	builder.WriteString(a.AssetType)
	builder.WriteString(", ")
	builder.WriteString("asset_details=")
	builder.WriteString(a.AssetDetails)
	builder.WriteString(", ")
