...
func GetAllAccessPolicyByAccountByTenant(client *ent.Client) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if isSuperAdmin || isTenantAdmin {
			accessPolicies, err = client.AccessPolicy.Query().Where(accesspolicy.HasAccountWith(account.And(account.ID(accountID), account.HasTenantWith(tenant.IDEQ(tenantID))))).All(c.Request().Context())
		} else {
