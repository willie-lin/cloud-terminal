...
func LoginUser(client *ent.Client) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		user, err := client.User.Query().Where(user.EmailEQ(u.Email)).Only(ctx)
...
func RegisterUser(client *ent.Client) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		user, err := client.User.Create().SetEmail(u.Email).SetPassword(hashedPassword).Save(ctx)
		if err != nil {
